...
export default function Login({ onLogin }: LoginProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (username.trim()) {
      ...
      setTimeout(() => {
        ...
        onLogin(username);
        toast.success('Welcome to DesignFuse! 🎨');
        setIsLoading(false);
      }, 1000);
    } else {
  ...
  const handleGoogleLogin = () => {
    ...
    setTimeout(() => {
      ...
      if (rememberMe) {
        localStorage.setItem('rememberMe', 'true');
        localStorage.setItem('savedUsername', googleUser);
      }
  ...
  const handleAppleLogin = () => {
    ...
    setTimeout(() => {
      const appleUser = 'Apple User';
      if (rememberMe) {
        localStorage.setItem('rememberMe', 'true');
        localStorage.setItem('savedUsername', appleUser);
      }
      ...
      <Card className="w-full max-w-md bg-slate-800 border-slate-700 shadow-2xl">
        <CardHeader className="text-center space-y-4">
          <div className="flex items-center justify-center space-x-3">
            ...
            />
            <CardTitle className="text-2xl font-bold text-teal-400">DesignFuse</CardTitle>
          </div>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-3">
            <Button
              ...
              disabled={isLoading}
              variant="outline"
              className="w-full bg-white hover:bg-gray-50 text-gray-900 border-gray-300 font-medium py-6"
            >
            ...
            <Button
              ...
              disabled={isLoading}
              variant="outline"
              className="w-full bg-black hover:bg-gray-900 text-white border-gray-600 font-medium py-6"
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Input
                  ...
                  className="bg-slate-700 border-slate-600 text-white placeholder:text-slate-400 focus:border-teal-500 focus:ring-teal-500 pr-10"
                />
                <button
                  type="button"
            ...
            <Button
              type="submit"
              disabled={isLoading}
              className="w-full bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-600 hover:to-cyan-600 text-white font-medium py-6 text-lg disabled:opacity-50"
            >
